...
n_tokens = len(word_vectors.vocab) # number of tokens
...
# tokens,tokenss2idx,n_tokens = get_val2idx(train_data[1].values)
tokens,tokens2idx,n_tokens = get_val2idx(word_vectors.vocab)
# embeddings = get_word2vecEmbedding(w2vmodel_path,tokens)
...
# n_sent_train= int(train_data[0].values[-1].split(':')[-1]) + 1
# n_sent_test = int(test_data[0].values[-1].split(':')[-1]) + 1 - n_sent_train
...
wv_matrix = (np.random.rand(n_tokens, vec_dimension) - 0.5) / 5.0
...
dense = TimeDistributed(Dense(n_tags, activation="softmax"))(output_drop)  # softmax output layer
...
model.compile(
    optimizer=Adam(lr=0.016, decay=0.001),
